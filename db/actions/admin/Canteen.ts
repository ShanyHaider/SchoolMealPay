...
export async function removeDailyMenu(
    ...
    if (result.length === 0) {
        throw new Error(`removeDailyMenu: no row found for ${menuDate} ${mealSlot} ${menuItemId}`);
    }
...
export async function createInventoryItem(raw: CreateInventoryItemInput) {
    ...
    const [created] = await db.insert(inventoryItemsTable).values(data).returning();
...
export async function updateInventoryQuantity(id: string, quantity: string) {
    await assertRole(["school_admin", "canteen_staff"]);
    const { quantity: validated } = updateInventoryQuantitySchema.parse({ quantity });
