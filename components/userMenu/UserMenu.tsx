...
// ✅ BillingTab and NotificationsTab removed — received as ReactNode props instead
...
interface UserMenuProps {
  isLoaded: boolean;
  // ✅ Server-rendered tabs passed down as opaque ReactNode
  billingTab: React.ReactNode;
  notificationsTab?: React.ReactNode;
}

export function UserMenu({ isLoaded, billingTab, notificationsTab }: UserMenuProps) {
  const { user } = useUser();
  ...
  if (!isLoaded || !user) {
    return <div className="h-11 w-28 animate-pulse rounded-full bg-zinc-200/50 dark:bg-zinc-800/50" />;
  }
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <div className="h-6.5 w-6.5 overflow-hidden rounded-full border border-zinc-300/50 bg-zinc-300 dark:bg-zinc-700 flex items-center justify-center shrink-0">
          {user.imageUrl
            ? <img src={user.imageUrl} className="h-full w-full object-cover" alt="avatar" />
            : user.firstName?.[0]}
        </div>
        <span className="max-w-25 truncate text-sm">{user.firstName || "Account"}</span>
        ...
        {isOpen && (
          ...
          >
            <div className="px-3 py-2.5 border-b border-zinc-100 dark:border-zinc-900 mb-1">
              <p className="text-xs font-bold text-zinc-900 dark:text-zinc-50 truncate">{user.fullName}</p>
              <p className="text-[10px] text-zinc-400 truncate mt-0.5">{user.primaryEmailAddress?.emailAddress}</p>
            </div>
            ...
            <div className="space-y-0.5">
              ...
              <button
                onClick={async () => { setIsOpen(false); await signOut({ redirectUrl: "/" }); }}
            ...
            <div className="fixed inset-0 z-9999 flex items-center justify-center p-2 sm:p-4 isolate">
              ...
              >
                ...
                <div className="w-full md:w-56 border-b md:border-b-0 md:border-r border-zinc-100 bg-zinc-50/50 dark:border-zinc-900 dark:bg-zinc-950/40 shrink-0 flex flex-col">
                  <div className="flex items-start justify-between px-5 pt-5 pb-0 md:pt-8 md:pb-0">
                    <div className="px-2 mb-3 md:mb-6">
                      <h2 className="text-lg md:text-xl font-black tracking-tight text-zinc-950 dark:text-white">Settings</h2>
                    </div>
                  ...
                  <nav className="flex md:flex-col gap-1 overflow-x-auto style-scroll-hidden md:overflow-visible px-4 md:px-5 pb-4 md:pb-5">
                    {TAB_CONFIG
                      // ✅ Hide notifications tab entirely if no content was passed
                      .filter((t) => t.id !== "notifications" || !!notificationsTab)
                ...
                <div className="flex-1 p-5 sm:p-8 md:p-10 pt-8 overflow-y-auto style-scroll-hidden">
                  ...
                  {/* ✅ Render pre-fetched ReactNode — no server imports needed */}
      ...
      <style jsx global>{`
        .style-scroll-hidden::-webkit-scrollbar { display: none; }
        .style-scroll-hidden { -ms-overflow-style: none; scrollbar-width: none; }
      `}</style>
    </div>
  );
}