  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-4 p-4 bg-[var(--bg-card)] border border-[var(--border-card)] rounded-2xl shadow-sm">
          <div className="flex items-center gap-2 px-4 py-2 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-card)]">
        ...
        {/* Menu Sections */}
        {menuItems.length === 0 ?
          <div className="flex flex-col items-center justify-center py-20 bg-[var(--bg-card)] border border-[var(--border-card)] rounded-2xl shadow-sm italic text-[var(--text-muted)]">
        ...
        : Object.entries(grouped).map(([slot, items]) => (
            <div key={slot} className="space-y-4">
              <div className="flex items-center gap-3">
                <h3 className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] px-1">
                  {slot}
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {items.map((item) => {
                  ...
                  return (
                    <div
                      key={menuItem.id}
                      className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between"
                    >
                      <div>
                        ...
                        <p className="text-sm text-[var(--text-secondary)] font-medium line-clamp-2 mb-4">
                      ...
                      <div className="flex items-center justify-end gap-3 pt-4 border-t border-[var(--border-card)]">
                        {qty > 0 ?
                          <div className="flex items-center gap-4 bg-[var(--bg-secondary)] rounded-xl p-1.5 border border-[var(--border-card)]">
                            <button
                              onClick={() => removeFromCart(menuItem.id)}
                              className="w-8 h-8 rounded-lg flex items-center justify-center bg-[var(--bg-card)] hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors text-[var(--text-primary)]"
                            >
                              <Minus size={14} />
                            </button>
      ...
      <div className="w-full lg:w-80 shrink-0">
        <div className="sticky top-24 bg-[var(--bg-card)] border border-[var(--border-card)] rounded-2xl p-6 shadow-sm flex flex-col gap-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-3 py-3 text-sm font-bold rounded-xl border border-[var(--border-card)] bg-[var(--bg-secondary)] text-[var(--text-primary)] outline-none focus:border-zinc-400 transition-all cursor-pointer"
            >
              {students.map((s) => (
          ...
          <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
            {cart.length === 0 ?
              <div className="text-center py-6">
                <Utensils
                  size={24}
                  className="mx-auto mb-2 text-[var(--text-muted)] opacity-20"
                />
            ...
            : cart.map((item) => (
                ...
                >
                  <div className="flex-1 pr-2">
                    ...
                    <p className="text-[10px] font-bold text-[var(--text-muted)] mt-0.5 uppercase">
          ...
          <div className="pt-4 border-t border-[var(--border-card)] space-y-4">
            ...
            <p className="text-[9px] text-[var(--text-muted)] text-center font-bold uppercase tracking-tighter">
