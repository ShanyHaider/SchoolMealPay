    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto">
            ...
            {STATUS_STEPS.map((step, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {!isLast && (
                    <div
                      className={`h-0.5 flex-1 mb-5 mx-1 rounded-full transition-colors ${
          ...
          <div key={item.id} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-(--text-muted)">
                {formatPKR(item.menuItem?.price ?? "0")} each
              </p>
            </div>
            <div className="text-right shrink-0">
              <p className="text-sm font-semibold text-(--text-primary)">
                {formatPKR(Number(item.menuItem?.price ?? 0) * item.quantity)}
              </p>
      ...
      <div className="p-5 bg-(--bg-card) border border-(--border-card) rounded-xl shadow-(--shadow-card) flex flex-col gap-3">
        ...
        <div className="flex justify-between text-sm font-semibold">
          ...
          <span className="text-(--text-primary)">{formatPKR(total)}</span>
        </div>
      </div>
