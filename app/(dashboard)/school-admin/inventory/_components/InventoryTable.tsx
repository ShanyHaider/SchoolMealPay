...
function StatusBadge({
  ...
  return (
    <span
      ...
      style={
        isCritical ? { background: "#ef444420", color: "#ef4444" }
        : isLow ?
...
export function InventoryTable({
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border text-center py-16" style={cardStyle}>
        <Package
          size={32}
    ...
    <>
          ...
          <tbody>
            {items.map((item) => {
              ...
              return (
                <tr
                  key={item.id}
                  className="border-t transition-colors hover:bg-(--bg-secondary)"
      ...
      <div className="md:hidden space-y-2">
        {items.map((item) => {
          const current = parseFloat(item.quantity);
          const reorder =
            item.lowStockThreshold ? parseFloat(item.lowStockThreshold) : null;
          const isLow = reorder !== null && current <= reorder;
          const isCritical = reorder !== null && current <= reorder * 0.5;
