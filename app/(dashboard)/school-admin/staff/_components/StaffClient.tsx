...
import { inviteStaffSchema, type InviteStaffInput } from "@/lib/validations/canteen";
    ...
    >
      {tabs.map((t) => (
        <button
          ...
          style={
            active === t.id
              ? { background: "var(--bg-card)", color: "var(--text-primary)", boxShadow: "var(--shadow-btn)" }
...
export function StaffClient({
  ...
  const {
    assignPendingId, handleAssign,
    removingStaff, setRemovingStaff, isRemovePending, confirmRemove,
    togglingStaff, setTogglingStaff, isTogglePending, confirmToggleDisable,
    deletingStaff, setDeletingStaff, isDeletePending, confirmDelete,
    anyActionPending,
  ...
  const { register, handleSubmit, reset: resetInviteForm, control, watch, formState: { errors } } =
    useForm<InviteStaffInput>({ resolver: zodResolver(inviteStaffSchema) });

  const closeInviteModal = () => { resetInviteForm(); setPromotable(null); setShowInvite(false); };
    ...
    startInviteTransition(async () => {
      try {
        ...
        if (result?.error) {
          ...
          if (existingId) setPromotable({ id: existingId, email: data.email, currentRole: existingRole ?? "parent" });
          const first = err.email?.[0] ?? Object.values(err).flat()[0];
          toast(typeof first === "string" ? first : "Failed to send invitation.", "error");
          return;
        }
  ...
  const isSubmitDisabled = isFormBusy || !nameValue?.trim() || !emailValue?.trim();
    ...
    <>
      <div className="space-y-4 sm:space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <TabBar active={activeTab} onChange={setActiveTab} pendingCount={pendingInvitations.length} />
          <button
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                disabled={isFormBusy}
                className="p-1.5 rounded-lg hover:bg-[var(--bg-tertiary)] transition-colors disabled:opacity-40 flex-shrink-0"
              >
            ...
            {promotable && (
              <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(251,191,36,0.07)", border: "1px solid rgba(251,191,36,0.2)" }}>
                <div className="flex items-start gap-2">
                  <ArrowUpCircle size={15} style={{ color: "#f59e0b", marginTop: 1, flexShrink: 0 }} />
                  <div>
                    <p className="text-xs font-medium" style={{ color: "#f59e0b" }}>Account already exists</p>
                    <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                      <span className="font-medium" style={{ color: "var(--text-secondary)" }}>{promotable.email}</span>{" "}
                      is already registered as a <span className="font-medium">{promotable.currentRole}</span>.
                ...
                <button
                  ...
                  style={{ background: "rgba(251,191,36,0.15)", color: "#f59e0b", border: "1px solid rgba(251,191,36,0.25)" }}
                >
                  {isPromotePending ? <Loader2 size={12} className="animate-spin" /> : <ArrowUpCircle size={12} />}
                  {isPromotePending ? "Promoting…" : "Promote to Canteen Staff"}
                </button>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit(onInvite)} className="space-y-3">
              <Field label="Full Name" required error={errors.name?.message}>
                <input {...register("name")} placeholder="Sarah Ahmed" style={inputStyle} disabled={isFormBusy} autoFocus />
              </Field>
              <Field label="Email Address" required error={errors.email?.message}>
                <input {...register("email")} type="email" placeholder="sarah@school.edu" style={inputStyle} disabled={isFormBusy} />
              </Field>
              <Field label="Phone (optional)" error={errors.phone?.message}>
                <input {...register("phone")} placeholder="+92 300 0000000" style={inputStyle} disabled={isFormBusy} />
              </Field>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  style={{ borderColor: "var(--border-input)", background: "var(--bg-tertiary)", color: "var(--text-secondary)" }}
                >
                  Cancel
                </button>
                <button
                ...
                >
                  {isInvitePending && <Loader2 size={14} className="animate-spin" />}
      ...
      {togglingStaff && createPortal(
        <ConfirmModal
          title={isCurrentlyDisabled ? "Enable staff member" : "Disable staff member"}
          description={
            isCurrentlyDisabled
