        ...
        startTransition(async () => {
            try {
                if (isEditing) {
                    ...
                    const parsed = createMenuItemSchema.parse({ canteenId, ...basePayload });
                    await createMenuItem(parsed);
                    onSuccess(`"${basePayload.name}" added to catalogue.`);
                }
                onClose();
            } catch {
                onError(isEditing ? "Failed to update menu item." : "Failed to add menu item.");
            }
        });
    };
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-3">
                    ...
                    <Field label="Category" error={errors.category?.message}>
                        <CustomSelect
                            ...
                            onChange={(v) => setValue("category", v as ItemCategory, { shouldDirty: true })}
                            disabled={isPending}
                        />
                    </Field>
