...
export function ScheduleTab({
    ...
    const menuItemOptions: DropdownOption[] = menuItems.map((mi) => ({
        value: mi.id,
        label: `${mi.name} — Rs. ${parseFloat(mi.price).toFixed(0)}`,
        icon: CATEGORY_ICONS_JSX[mi.category as keyof typeof CATEGORY_ICONS_JSX] ?? null,
    }));
    ...
    const handleSchedule = (menuItemId: string, date: string) => {
        ...
        if (!result.success) {
            onError(result.error.issues[0]?.message ?? "Scheduling validation failed.");
            return;
        }
        ...
        (async () => {
            try {
                ...
                // Optimistic add
                const fresh = await getWeeklyMenuAction(selectedCanteen, weekStart, weekEnd);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex rounded-lg p-1 gap-1" style={{ background: "var(--bg-pill)" }}>
                    {MEAL_SLOTS.map((slot) => (
                        <button
                            key={slot}
                            ...
                            style={{
                                background:
                                    selectedSlot === slot ? "var(--bg-pill-active)" : "transparent",
                                color:
                                    selectedSlot === slot
                                        ? "var(--text-primary)"
                                        : "var(--text-secondary)",
